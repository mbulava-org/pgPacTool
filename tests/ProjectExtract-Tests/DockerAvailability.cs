...
/// Call SkipIfDockerUnavailable() at the top of any [SetUp] or [OneTimeSetUp] that
/// launches a container, so tests are gracefully ignored rather than failing with
/// DockerUnavailableException when Docker is not present in the environment.
/// </summary>
internal static class DockerAvailability
{
    ...
    public static bool IsAvailable
    {
        get
        {
            ...
            // Check for the Docker socket / named pipe
    ...
    /// <summary>
    /// Calls Assert.Ignore() if Docker is unavailable, causing the test (or the
    /// entire fixture when called from [OneTimeSetUp]) to be skipped rather than
    /// failing with DockerUnavailableException.
    /// </summary>
    ...
    /// <summary>
    /// Returns true if an exception is a Testcontainers DockerUnavailableException.
    /// Useful as a when-clause guard in catch blocks.
    /// </summary>
    public static bool IsDockerUnavailableException(Exception ex) =>
        ex.GetType().Name.Contains("DockerUnavailable", StringComparison.Ordinal) ||
